...
class Parameter:
    ...
    def mutate(self, curr_value: Union[int, float],
               scale: float) -> Union[int, float]:
        ''' Parameter.mutate: mutates current parameter value by using the
        equation:

        V = X + rand(-scale, scale) * (X - Xrand)
        ...
        Args:
            ...
            scale (float): scalar to mutate parameter by, [-1, 1]
        ...
        new_value = self._dtype(
            curr_value + uniform(-scale, scale) * (curr_value - self.rand_val)
        )
        ...
        if new_value == curr_value:
            if self._dtype is int and self._max_val - self._min_val <= 2:
                ...
                return self.mutate(curr_value, scale)
